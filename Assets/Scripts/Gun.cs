...
public class Gun : MonoBehaviour
{
	...
	private void Update()
	{
		if (!PlayerMovement.Instance || PlayerMovement.Instance.HasGun())
		{
			...
			Quaternion b = Quaternion.LookRotation((PlayerMovement.Instance.GetGrapplePoint() - transform.position).normalized);
			rotationOffset += Mathf.DeltaAngle(transform.parent.rotation.eulerAngles.y, prevRotation.y);
			...
			{
				b = Quaternion.Euler(transform.parent.rotation.eulerAngles - vector);
			}
			...
			transform.localPosition = Vector3.SmoothDamp(transform.localPosition, vector3 + desiredBob, ref posVel, posSpeed);
